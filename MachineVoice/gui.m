...
if isempty(R_samp_len)
 ...
 set(handles.tx_msg,'ForegroundColor',[1 0 0],'string','Recording..');
	...
	data=wavrecord(96000*R_samp_len,96000);% recording the sound
...
% handles    structure with handles and user data (see GUIDATA)
   ...
   if ~((ymin > ymax) | (xmin > xmax))
      ...
      imin = round(xmin*96000)+1;
      imax = round(xmax*96000)+1;
  end
    if length(signal(imin:imax) )~= 0
        %Sail
        player = audioplayer(signal(imin:imax),96000);
        play(player);
        pause(5);
    end
...
[filename, pathname] = uiputfile('*.wav', 'Save Data to Wave File');
	if filename ~= 0
		wavwrite(signal,96000,[pathname filename])
	end
...
% check file is selected
 ...
 signal = audioread([pathname filename]);
