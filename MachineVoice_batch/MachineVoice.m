...
if featureIndex==1
    ...
    for k=3:size(fileList,1)
        ...
        if (length(signal)<10)|(length(mhs_file)<2)
            ...
            match_v(i_max_2)=[];             % Remove it to next value
            [max_3 i_max_3]=max(match_v);   
            wavFile{k-2}=fileList(k,:);
            ClosestMatchVoice{k-2}=name(i_max_1,:);
            MatchValue(k-2)=max_1;
            Max2_Voice{k-2}=name(i_max_2,:);
            Max2_Value(k-2)=max_2;
            Max3_Voice{k-2}=name(i_max_3,:);
            Max3_Value(k-2)=max_3;
            %Now Deside on the Quality     
            %match_v array contains the value of mathces with various features
            if max_1>-13.6
                Quality{k-2}='PERFECT';
...
wavFile=wavFile';
Quality=Quality';
ClosestMatchVoice=ClosestMatchVoice';
MatchValue=MatchValue';
Max2_Voice=Max2_Voice';
Max2_Value=Max2_Value';
Max3_Voice=Max3_Voice';
Max3_Value=Max3_Value';
theTable=table(wavFile,Quality,ClosestMatchVoice,MatchValue,Max2_Voice,Max2_Value,Max3_Voice,Max3_Value);
writetable(theTable,'SpeakerRecogonitionResult.xls')
% --------------------------------------------------------------------
